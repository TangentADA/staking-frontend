...
export default function StakingPool({
  ...
  console.log("${getStakingAddress(poolInfo.script.script)}", getStakingAddress(poolInfo.script.script))
  ...
  const {
    ...
    userStaked,
    pendingRewards,
    estimatedPerEpochRewards,
    load,
  } = usePoolData(
    poolInfo.script.script,
    poolInfo.oldScript.script,
    BigInt(poolInfo.rewardPerEpochQt / 1000000),
    poolInfo.stakingUnit,
    poolInfo.stakingPolicy,
    poolInfo.harvestUnit,
    poolInfo.poolIndex
  )
  ...
  const harvest = async () => {
    ...
    console.log(
      `About to call /api/${
        stakingPoolInfo.poolInfo.poolIndex
      }/${await loadedLucid.wallet.address()}/harvestTx`
    )
    ...
    console.log(
      `About to call /api/0/${await loadedLucid.wallet.address()}/${sig}`
      )
      console.log({
        poolIndex: poolI,
        address: await loadedLucid.wallet.address(),
  ...
  const withdrawAct = async (harvest: boolean, value: Assets | null = null) => {
    ...
    let utxos1 = await searchStakes(
      pkh,
      await loadedLucid.utxosAt(stakingAddress)
    )
    let utxos2 = await searchStakes(
      pkh,
      await loadedLucid.utxosAt(stakingAddress2)
    )
    let utxos = utxos1.concat(utxos2)
    if (!utxos || utxos.length < 1)
      throw harvest ? 'Nothing to harvest' : 'Nothing to withdraw'
    let contract = 0
    ...
    try {
      const tx: TxComplete = await withdrawTx(
        utxos,
        await loadedLucid.wallet.address(),
        pkh,
        harvest,
        value,
        contract
  ...
  async function withdrawTx(
    ...
    pk: PaymentKeyHash,
    onlyCollect: boolean,
    withdrawValue: Assets | null,
    contract: number = 1
  ): Promise<TxComplete> {
    const lucid = await initializeLucid(
      await window.cardano[walletName].enable()
    )
    let tx = new Tx(lucid).collectFrom(utxos, WITHDRAW())
    ...
    <div className="card my-2 bg-base-100 shadow-xl">
      <div className="grid min-w-full grid-cols-2">
        <div className="p-6 text-left">
          Total {poolInfo.rewardPerEpochQt / 1000000} {poolInfo.reward}
        </div>
        <div className="grid justify-items-end p-6">
          <div
            className={`w-90 grid grid-cols-${
              estimatedPerEpochRewards ? 1 : 2
            }`}
          >
            Estimated {userInfo.rewardType} per epoch:{' '}
            {estimatedPerEpochRewards || (
              <Skeleton className="ml-2" baseColor="#2A4B89" />
            )}
          </div>
        </div>
      </div>
      <div className="card-body mt-0 grid min-w-full grid-cols-1 pt-0 md:grid-cols-3">
        <div>
        ...
        <div>
          ...
          <p>
            Staked:{' '}
            {totalStaked || <Skeleton width={'50%'} baseColor="currentColor" />}
          </p>
          <p>
            Stakers:{' '}
            {totalStakers || (
              <Skeleton width={'50%'} baseColor="currentColor" />
            )}
          </p>
          <div className="pt-1">
            Contract Address
            <button
              onClick={() =>
                copyToClipboard(getStakingAddress(poolInfo.script.script))
              }
            ...
            <a
              target={'_blank'}
              href={`https://cardanoscan.io/address/${getStakingAddress(
                poolInfo.script.script
              )}`}
              className="btn btn-ghost btn-circle"
            >
        ...
        <div className="card-actions justify-end">
          <div className="stats grid min-w-full grid-rows-2 bg-secondary text-base-100 md:grid-cols-2 md:grid-rows-1">
            <div className="stat">
              <div className="stat-title">
                {userInfo.rewardType} Pending Reward
              </div>
              <div className="stat-value">
                {!pendingRewards ? (
                  ...
                  <div className="flex">
                    {pendingRewards.toString().split('.')[0]}
                    <p className="mt-auto text-xs">
                      .{pendingRewards.toString().split('.')[1]}
                    </p>
                  </div>
                ) : (
                  pendingRewards
            ...
            <div className="stat">
              ...
              <div className="stat-value">
                {userStaked || <Skeleton baseColor="#2A4B89" />}
              </div>
              <div className="stat-actions">
                {poolInfo.type === 'NFT' ? (
                  <>
                    <NftStakeModalBtn
                      contractAddress={stakingAddress}
                      policyid={
                        poolInfo.stakingPolicy ? poolInfo.stakingPolicy : ''
                      }
                      enabled={walletEnabled}
                      action={withdraw}
                      actionName="Withdraw"
                  ...
                  <>
                    <StakeModalBtn
                      ...
                      stakingUnit={
                        poolInfo.stakingUnit ? poolInfo.stakingUnit : ''
                      }
                      enabled={walletEnabled}
                      action={withdraw}
                      actionName="Withdraw"
